...
function Interview() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [technical, behavioral, coding, tips] = await Promise.all([
        interviewService.getTechnicalQuestions(),
        interviewService.getBehavioralQuestions(),
        interviewService.getCodingChallenges(),
        interviewService.getPreparationTips(),
      ]);

      setData({ technical, behavioral, coding, tips });
    } catch (err) {
      setError(
        err?.response?.data?.message ||
          "Failed to load interview preparation content. Please try again."
      );
    } finally {
      setLoading(false);
    }
